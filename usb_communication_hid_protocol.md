...
* 数据传输方式
设备是被动设备，需要PC进行一次“写读”来完成一次数据交互，数据发起方只能是PC。PC每次发起数据传输，usb都会发送指令到设备"CTL 21 09 00 02  00 00 88 00  SET REPORT",设备在中断中进行接收。
```c
// SET REPORT
if(SetupBuff->bmRequestType == 0x21)
{
    if(SetupBuff->bRequest == 0x09)
    {
        ep0num = SetupBuff->wLength.Lo + (SetupBuff->wLength.Hi << 8);
        if(ep0num == 0x88)
        {
            //接收端点0数据
        }
...
136  IN     90 00 01 01  4f 6e 65 4b  65 79 00 00  00 00 00 00  ....OneKey......  3.0ms        22.1.0
            ...
            00 00 00 00  30 37 30 30  30 32 30 30  39 34 00 ff  ....0700020094..               22.1.64
...
136  OUT    00 00 00 00  00 00 00 00  00 00 00 00  00 00 00 00  ................  501us        12.2.0
            ...
            ...
     CTL    21 09 00 02  00 00 88 00                            SET REPORT         14ms        12.1.0
136  OUT    00 00 00 00  00 00 00 00  00 00 00 00  00 00 00 00  ................  501us        12.2.0(3)
            ...
            ...
     CTL    21 09 00 02  00 00 88 00                            SET REPORT         14ms        12.1.0    
136  OUT    00 00 00 00  00 00 00 00  00 00 00 00  00 00 00 00  ................  501us        12.2.0
